...
/**
 * Парсер, который из фразы делает сумму, ставку и срок.
 *
 * Шаг 1. Ищем срок: ищем число начиная от слова год или лет или месяц или мес
 * и заканчивая нецифрой. В цифре может быть запятая — тогда меняем её на точку.
 * Запоминаем строку и значение этой строки.
 *
 * Шаг 2. Ищем ставку: всё до слова процент или значка % — это ставка.
 * Запоминаем цифру и значение найденной строки.
 *
 * Шаг 3. Ищем цифру перед словом «руб». Если не нашли — берём исходную строку,
 * удаляем найденные в пункте 1 и 2 строки, потом оставляем только цифры:
 * удаляем все нецифры, в том числе точку. Это и будет наша сумма.
 */
