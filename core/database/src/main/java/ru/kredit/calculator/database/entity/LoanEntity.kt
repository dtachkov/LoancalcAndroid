...
data class LoanEntity(
    ...
    @ColumnInfo(defaultValue = "0", name = DatabaseContract.LoanColumns.APPLY_EXTRAS_IMMEDIATELY)
    val applyExtrasImmediately: Int? = 0,
    @ColumnInfo(
        defaultValue = "1",
        name = DatabaseContract.LoanColumns.CALCULATE_EXTRAS_BY_TERM_LIKE_SBERBANK,
    )
    val calculateExtrasByTermLikeSberbank: Int? = 1,
    @ColumnInfo(
        defaultValue = "0",
        name = DatabaseContract.LoanColumns.IGNORE_PASSED_PERIODS_AFTER_RATE_CHANGE,
    )
    val ignorePassedPeriodsAfterRateChange: Int? = 0,
    @ColumnInfo(defaultValue = "0", name = DatabaseContract.LoanColumns.EXTRA_DAY_IN_MONTH)
    val extraDayInMonth: Int? = 0,
    @ColumnInfo(defaultValue = "0", name = DatabaseContract.LoanColumns.IS_FORECAST_ACTIVE)
    val isForecastActive: Int? = 0,
    @ColumnInfo(name = DatabaseContract.LoanColumns.FORECAST_MONTHLY_PAY)
    val forecastMonthlyPay: Float? = null,
